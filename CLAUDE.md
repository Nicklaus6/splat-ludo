...
- **`public/index.html`**: portal homepage — a card grid listing all games. **`public/games/ludo/index.html`** (~1500 lines): the entire Ludo game as one file — SVG board rendering, animations, Web Audio sound synthesis, Ludo rules, CPU AI, and networking. Zero frontend dependencies. **`public/{login,register}.html`**: static auth pages. New games follow the same pattern: one self-contained HTML file under `public/games/<name>/`, listed as a card on the portal.
- **`public/games/tamagotchi/index.html`**: 口袋蛋仔 — single-player virtual pet (canvas pixel LCD, pixel-art icons, Web Audio beeps). The only game with an external dep: three.js via CDN import map (jsdelivr, pinned version) renders the egg shell in WebGL (`initEgg3D`: LatheGeometry + MeshPhysicalMaterial clearcoat + RoomEnvironment; shell painted into a CanvasTexture so skins swap without assets). DOM screen/buttons overlay the canvas and tilt in sync via CSS transform; 3D init failure is non-fatal (game still playable). All simulation is client-side, ticked per-minute in `applyElapsed` so offline time counts (catch-up capped at 3 weeks). Shell colorways are a skin system: `SKINS` config (shell gradient/spots/buttons/LCD palette) + `applySkin()`; adding a new generation/colorway = one `SKINS` entry, choice persists in `pet.skin`. Per-user save via `GET/POST /api/games/tamagotchi/pet` (`pets` table, one JSONB row per user, upsert); saved on every action + 60s interval + `sendBeacon` on tab hide. Loading merges the saved blob into `newEgg()` defaults, so adding fields never breaks old saves.
